...
class AtresPlayerIE(InfoExtractor):
    _VALID_URL = r'https?://(?:www\.)?atresplayer\.com/(?:[^/]+/){3,4}(?P<display_id>[^/]+?)_(?P<id>[0-9a-f]{24})'
    ...
    def _real_extract(self, url):
        ...
        preloaded_state = self._parse_json(self._html_search_regex(
            r'window\.__PRELOADED_STATE__\s*=\s*(\{(.*?)\});', page, 'preloaded state'), video_id)
